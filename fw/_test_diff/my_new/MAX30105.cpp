...
uint16_t MAX30105::getGreen(void)
{
  //Check the sensor for new data for 250ms
  if(safeCheck(250))
  ...
  if (readPointer != writePointer)
  {
    ...
    while (bytesLeftToRead > 0)
    {
      ...
      while (toGet > 0)
      {
        ...
        //sense.red[sense.head] = (uint16_t)(tempLong >> 2); //keep MSB
        sense.red[sense.head] = (uint16_t)(tempLong & 0xFFFF); //keep LSB
      ...
      //sense.IR[sense.head] = (uint16_t)(tempLong >> 2);
      sense.IR[sense.head] = (uint16_t)(tempLong & 0xFFFF);
        ...
        if (activeLEDs > 2)
        {
          ...
          //sense.green[sense.head] = (uint16_t)(tempLong >> 2);
          sense.green[sense.head] = (uint16_t)(tempLong & 0xFFFF);
